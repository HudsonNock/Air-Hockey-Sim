...
def collect_data():
    ...
    for idx in range(50):
        ...
        action_commands.append(np.concatenate((xf, np.array([(13-3)*np.random.random() + 3.0, (13-3)*np.random.random() + 3.0]), np.array([(0.2-0.13)*np.random.random() + 0.13])), axis=0))
    ...
    while True:
        ...
        if signal_end:
            ...
            #print("------")
            #print(dts)
            with open("new_data/mallet_data_supercap_random_13V_Fx.csv", "w", newline="") as f:
                writer = csv.writer(f)
                # Write header
