...
def collect_data():
    ...
    for idx in range(50):
        ...
        action_commands.append(np.concatenate((xf, np.array([(10-3)*np.random.random() + 3.0, (10-3)*np.random.random() + 3.0]), np.array([(0.2-0.13)*np.random.random() + 0.13])), axis=0))
    ...
    while True:
        ...
        if signal_end:
            ...
            #print("------")
            #print(dts)
            with open("new_data/mallet_data_random_supercap_feedback_17_10V.csv", "w", newline="") as f:
                writer = csv.writer(f)
                # Write header
