...
def system_loop(cam, load):
    ...
    action_commands = np.zeros([6000, 5])
    idx = 0
    ...
    try:
        while True:
	        ...
	        if not no_update:
	            ...
	            #Vo[0] = np.minimum(Vo[0], 10)
	            #Vo[1] = np.minimum(Vo[1], 10)
	            obs[28:30] = xf
	            obs[30:32] = Vo
	            #print("A")
	            #print(xf)
	            ...
	            time_diff = new_time - timer
	            timer = new_time
	            action_commands[idx, :] = np.concatenate([obs[28:32], np.array([time_diff])], axis=0)
	            idx += 1
        ...
        np.save("new_data/occilation_actions_np.npy", action_commands[:idx])
