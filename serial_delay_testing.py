...
def lock_memory():
    ...
    if result != 0:
        print(f"mlockall failed: {result}")
    else:
        print("Memory locked successfully")

def precise_timing_measurement(cam, num_frames=1200):
    """Optimized timing measurement with minimal overhead"""
    
    # Pre-allocate arrays
    frame_intervals = np.zeros(num_frames - 1, dtype=np.float64)
    ...
    try:
        ...
        for i in range(num_frames):
            ...
            if i > 0:
                frame_intervals[i-1] = (current_time - prev_time) * 1000  # Convert to ms
            
            # Minimal processing - just get array shape to ensure data is accessed
        ...
        for i in range(num_frames):
            ...
            frame_intervals[i] = ((end - start) * 1000)  # ms
            image.Release()
    ...
    std_dev = np.std(frame_intervals)
    ...
    for p in percentiles:
        val = np.percentile(frame_intervals, p)
        print(f"{p}th percentile: {val:.4f}ms")
    
    # Distribution analysis
    ranges = [(0, 1000/60 - 0.5), (1000/60 - 0.5, 1000/60+0.5), (1000/60+0.5, 100.0)]
    for min_r, max_r in ranges:
        count = np.sum((frame_intervals >= min_r) & (frame_intervals < max_r))
