...
//array<float,2> kp = {1200, 1200}; //{80,80};
//array<float,2> ki = {1200.0, 1200.0}; //{1.6,1.6};
//array<float,2> kd = {10,10}; //{1.6,6.4};

array<float,2> kp = {330.8, 1285}; //{80,80};
array<float,2> ki = {377.3, 4554}; //{1.6,1.6};
array<float,2> kd = {72.5, 90.6}; //{1.6,6.4};
...
void send_motor_pos() {
  ...
  pkt.exp_pos_q[0] = quant(expected_xy_pos[0], -0.5, 2);  
  pkt.exp_pos_q[1] = quant(expected_xy_pos[1], -0.5, 2);
  pkt.pwm_q[0] = quant(pwm_vals[0], -1.1, 1.1);
  pkt.pwm_q[1] = quant(pwm_vals[1], -1.1, 1.1);
  pkt.dt_q = quant(dt_angle, 0.0, 3.0);
...
void apply_voltage(double t) {
  ...
  if (mode != 4) {
    ...
    //pwm_vals[0] = expected_xy_pos[0];
    //pwm_vals[1] = expected_xy_pos[1];
    ...
    //pwm_vals[0] = Vxy[0]/24.0;
    //pwm_vals[1] = Vxy[1]/24.0;
