...
load_filepath = "checkpoints/model_76.pth" #"checkpoints/model_10.pth"
second_actor_filepath = "yes"
save_filepath = "checkpoints/model_94.pth" #Best is 12, 15, 22, 44, 76
train = True
...
if load_filepath is not None:
    ...
    #policy_optimizer.load_state_dict(checkpoint['policy_optim_state_dict'])
    #value_optimizer.load_state_dict(checkpoint['value_optim_state_dict'])
...
if not train:
    coll_vars = np.array([[0.75, 0.01, 0.74, 0.7, 0.01, 0.69, 0.02, 0.75, 0.01, 0.74, 0.7, 0.01, 0.69, 0.02]])
...
block_reward = 20.0
actions_since_reset = np.random.randint(0, 60*59, (envs,))
...
if train:
    ...
    class TrajectoryBuffer:
        ...
        def add_step(self, obs, actions, rewards, dones, next_obs, log_probs):
            """Add a step to all environment trajectories"""
            batch_size = obs.shape[0]
            
            for env_idx in range(batch_size):
                if env_idx not in self.trajectories:
    ...
    for update in range(500000):  # Training loop
        ...
        for timestep in range(sim_steps):
            ...
            if second_actor_filepath is None:
                ...
                left_puck = obs[int(envs*2/3):envs,0] < bounds[0]/2
                actions_np[envs+int(envs*2/3):, 0][left_puck] = 0.12
                actions_np[envs+int(envs*2/3):, 1][left_puck] = (np.random.random((left_puck.shape)) * 0.2 + (bounds[1]/2 - 0.1))[left_puck]
                if (update*50+timestep) % 10 != 0:
                    actions_np[envs+int(envs*2/3):][left_puck] = np.copy(obs[envs+int(envs*2/3):,28:32][left_puck])
            ...
            apply_acctime = np.linalg.norm(obs[:,28:30] - obs[:,24:26], axis=1) > 0.3
            ...
            while True:
                ...
                if next_img < next_mallet and next_img < next_action:
                    ...
                    terminal_rewards[:envs] += np.where(cross_right > 0, 10 + 3*np.sqrt(cross_right), 0)
                    terminal_rewards[envs:] += np.where(cross_left > 0, 10 + 3*np.sqrt(cross_left), 0)
                    ...
                    rewards += 0.1*np.where(np.logical_not(puck_attack), 0.1-0.5*np.linalg.norm(obs[:,20:22] - defending, axis=1), 0)

                    break

            actions_since_reset += 1

            rewards -= np.linalg.norm(obs[:,20:22] - obs[:,24:26], axis=1) * 0.3
            ...
            if timestep == sim_steps - 1:
                too_long_reset = actions_since_reset > 60*9
            ...
            if num_resets > 0:
                actions_since_reset[dones[:envs]] = 0
        ...
        batch_td = torch.stack(all_processed_data).to('cuda')
...
for timestep in range(sim_steps):
    ...
    while True:
        ...
        if next_img < next_mallet and next_img < next_action:
            ...
            #rewards -= 0.04*np.where(puck_attack, np.linalg.norm(obs[:,:2] - obs[:,4:6], axis=1)*120 - 0.3, 0)
    ...
    rewards[(close_side & (obs[:,0] > bounds[0]/2))] += 10.0
