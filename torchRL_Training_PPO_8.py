...
save_filepath = "checkpoints/model_184.pth" #Best is 12, 15, 22, 44, 76, 103, 121, 134, 142, 160, 165, 178
train = False
...
if not train:
    ab_vars[0,0,:] = (ab_ranges[:,0] + ab_ranges[:,1])/2
        ...
        for timestep in range(sim_steps):
            with torch.no_grad():
                if second_actor_filepath is None:
                    ...
                    past_idxs = q_action_idxs.copy()
                    q_action_idxs[:] = 5
                    q_action_idxs[left_puck] = q_out['action'].detach().cpu().numpy()
                    q_action_idxs[left_puck] = np.where(np.random.rand(np.sum(left_puck)) < q_epsilon, np.random.randint(0,5, size=np.sum(left_puck)), q_action_idxs[left_puck])
                    actions_np[envs+int(envs/2):, :][left_puck] = action_map[q_action_idxs[q_action_idxs != 5]]

                    rewards[envs+int(envs/2):] -= np.where(np.logical_and(past_idxs != 5, q_action_idxs != 5), np.abs(past_idxs - q_action_idxs) / 3, 0)
            ...
            if second_actor_filepath is None:
                ...
                defender_step = np.logical_not(q_action_idxs == 5)

                td = TensorDict({
                        "observation": torch.tensor(past_obs[envs+int(envs/2):][defender_step], dtype=torch.float32),
                        "action": torch.tensor(q_action_idxs[defender_step][:,None], dtype=torch.int64),
                        "next": TensorDict({
                            "reward": torch.tensor(rewards[envs+int(envs/2):][defender_step][:,None], dtype=torch.float32),
                            "observation": torch.tensor(obs[envs+int(envs/2):][defender_step], dtype=torch.float32),
                            "done": torch.tensor(dones[envs+int(envs/2):][defender_step][:,None], dtype=torch.bool),
