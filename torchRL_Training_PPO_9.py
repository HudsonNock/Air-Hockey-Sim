...
obs_dim = 54 #[puck_pos, opp_mallet_pos] #guess mean low stdl
            #[mallet_pos, mallet_vel, past mallet_pos, past malet_vel, past action]
            #[e_n0, e_nr, e_nf, e_t0, e_tr, e_tf, e_std_n, e_std_t] x2 mallet and wall,
            #[a1, a2, a3, b1, b2, b3]
...
puck_std = np.array([[0,0.0005], [0,0.0005]]) # std in x y, a*x + b
puck_beam_std = np.array([[0,0.0005], [0,0.0005]]) #std in x y, a*x+b
percent_miss = 0.9
puck_perlin_mean = 0.005

camera_pos = [0.1, 1.3] #x,z
beam_width = 0.05
beam_thickness = 0.01
beam_height = 0.06 #from floor roof of beam
...
e_ni_mallet = [0.6, 0.9]
e_nf_mallet = [0.4, 0.42]
e_nr_mallet = [0, 0.25]
e_ti_mallet = [0.5, 0.52]
e_tf_mallet = [0.4, 0.42]
e_tr_mallet = [0, 0.25]
e_nstd_mallet = [0,0.05]
e_tstd_mallet = [0,0.05]
...
noise_map= generate_perlin_noise_2d(shape, res) * puck_perlin_mean
...
coll_vars = np.empty((envs, 16))
    
#[e_n0, e_nr, e_nf, e_t0, e_tr, e_tf, e_std_n, e_std_t]
coll_vars[:, 0] = np.random.uniform(e_ni_wall[0], e_ni_wall[1], (envs,))    # e_n0
coll_vars[:, 1] = np.random.uniform(e_nr_wall[0], e_nr_wall[1], (envs,))      # e_nr  
coll_vars[:, 2] = np.random.uniform(e_nf_wall[0], e_nf_wall[1], (envs,))
coll_vars[:, 3] = np.random.uniform(e_ti_wall[0], e_ti_wall[1], (envs,))
coll_vars[:, 4] = np.random.uniform(e_tr_wall[0], e_tr_wall[1], (envs,))
coll_vars[:, 5] = np.random.uniform(e_tf_wall[0], e_tf_wall[1], (envs,))
coll_vars[:, 6] = np.random.uniform(e_nstd_wall[0], e_nstd_wall[1], (envs,))
coll_vars[:, 7] = np.random.uniform(e_tstd_wall[0], e_tstd_wall[1], (envs,))

coll_vars[:, 8] = np.random.uniform(e_ni_mallet[0], e_ni_mallet[1], (envs,))    # e_n0
coll_vars[:, 9] = np.random.uniform(e_nr_mallet[0], e_nr_mallet[1], (envs,))      # e_nr  
coll_vars[:, 10] = np.random.uniform(e_nf_mallet[0], e_nf_mallet[1], (envs,))
coll_vars[:, 11] = np.random.uniform(e_ti_mallet[0], e_ti_mallet[1], (envs,))
coll_vars[:, 12] = np.random.uniform(e_tr_mallet[0], e_tr_mallet[1], (envs,))
coll_vars[:, 13] = np.random.uniform(e_tf_mallet[0], e_tf_mallet[1], (envs,))
coll_vars[:, 14] = np.random.uniform(e_nstd_mallet[0], e_nstd_mallet[1], (envs,))
coll_vars[:, 15] = np.random.uniform(e_tstd_mallet[0], e_tstd_mallet[1], (envs,))

if not train:
    coll_vars = np.array([[0.75, 0.01, 0.7, 0.7, 0.01, 0.65, 0.02, 0.02, 0.8, 0.01, 0.75, 0.74, 0.01, 0.73, 0.02, 0.02]])
    coll_vars = np.array([[0.802, 0.167, 0.566, 0.73, 0.1, 0.762, 0.13, 0.136, 0.802, 0.167, 0.566, 0.73, 0.1, 0.762, 0.13, 0.136]])

#coll_vars = np.array([[0.802, 0.167, 0.566, 0.1, 2.822, 0.762, 0.13, 0.336, 0.802, 0.167, 0.566, 0.1, 2.822, 0.762, 0.13, 0.336]])
...
if not train:
    ab_vars[0,0,:] = np.array([7.629e-06, 6.617e-03, 7.0e-02, -7.445e-06, -2.661e-03, 5.277e-03])
...
beam_interference[envs:] = np.logical_and((bounds-puck_pos)[:,0]+puck_r > beam_coeffs[0,0] * np.full((envs,2), mallet_init[0])[:,0] + beam_coeffs[0,1], (bounds-puck_pos)[:,0]-puck_r < beam_coeffs[1,0] * np.full((envs,2), mallet_init[0])[:,0] + beam_coeffs[1,1])
...
camera_buffer[:, envs:, :2] = np.tile(bounds-puck_pos+puck_noise, (camera_buffer_size,1,1)) +\
                                     np.stack((np.random.normal(0,\
                                                np.where(beam_interference[envs:], puck_beam_std[0,0] * (bounds-puck_pos)[:,0] + puck_beam_std[0,1], puck_std[0,0] * (bounds-puck_pos)[:,0] + puck_std[0,1]),\
                                                 (camera_buffer_size,envs)),\
                                              np.random.normal(0,\
                                                np.where(beam_interference[envs:], puck_beam_std[1,0] * (bounds-puck_pos)[:,0] + puck_beam_std[1,1], puck_std[1,0] * (bounds-puck_pos)[:,0] + puck_std[1,1]),\
                                                 (camera_buffer_size,envs))),\
                                              axis=-1)
...
while True:
    ...
    if next_img < next_mallet and next_img < next_action:
        ...
        opp_puck_wgn_noise = np.stack((np.random.normal(0, np.where(beam_interference[envs:], puck_beam_std[0,0] * (bounds[0] - puck_pos[:,0]) + puck_beam_std[0,1], puck_std[0,0] * (bounds[0] - puck_pos[:,0]) + puck_std[0,1]), (envs,)), np.random.normal(0, np.where(beam_interference[envs:], puck_beam_std[1,0] * (bounds[0] - puck_pos[:,0]) + puck_beam_std[1,1], puck_std[1,0] * (bounds[0] - puck_pos[:,0]) + puck_std[1,1]), (envs,))), axis=-1)

        puck_pos = np.concatenate([puck_pos + puck_noise + puck_wgn_noise, bounds - puck_pos + puck_noise + opp_puck_wgn_noise],axis=0)

        if beam_interference.any():
            past_puck_data = camera_buffer.get([0])[:,:2][beam_interference] # player, x/y (2*envs,2)
            miss_mask = np.random.random((int(np.sum(beam_interference)))) < percent_miss
            puck_pos[beam_interference][miss_mask] = past_puck_data[miss_mask]
        ...
        opp_mallet_wgn_noise = np.stack((np.random.normal(0, puck_std[0,0] * (bounds[0] - mallet_pos[:,0,0]) + puck_std[0,1], (envs,)), np.random.normal(0, puck_std[1,0] * (bounds[0] - mallet_pos[:,0,0]) + puck_std[1,1], (envs,))), axis=-1)
        op_mallet_pos = np.concatenate([mallet_pos[:,1,:] + mallet_noise[:,2:] + mallet_wgn_noise, bounds - mallet_pos[:,0,:] + mallet_noise[:,:2] + opp_mallet_wgn_noise], axis=0)
...
if train:
    ...
    for update in range(500000):  # Training loop
        ...
        for timestep in range(sim_steps):
            ...
            if second_actor_filepath is None:
                ...
                if defender_step.any():
                    td = TensorDict({
                            ...
                            "action": torch.tensor(q_action_idxs[defender_step][:,None], dtype=torch.int64),
                            "next": TensorDict({
            ...
            if num_resets > 0:
                coll_vars = np.empty((num_resets, 16))

                coll_vars[:, 0] = np.random.uniform(e_ni_wall[0], e_ni_wall[1], (num_resets,))    # e_n0
                coll_vars[:, 1] = np.random.uniform(e_nr_wall[0], e_nr_wall[1], (num_resets,))      # e_nr  
                coll_vars[:, 2] = np.random.uniform(e_nf_wall[0], e_nf_wall[1], (num_resets,))
                coll_vars[:, 3] = np.random.uniform(e_ti_wall[0], e_ti_wall[1], (num_resets,))
                coll_vars[:, 4] = np.random.uniform(e_tr_wall[0], e_tr_wall[1], (num_resets,))
                coll_vars[:, 5] = np.random.uniform(e_tf_wall[0], e_tf_wall[1], (num_resets,))
                coll_vars[:, 6] = np.random.uniform(e_nstd_wall[0], e_nstd_wall[1], (num_resets,))
                coll_vars[:, 7] = np.random.uniform(e_tstd_wall[0], e_tstd_wall[1], (num_resets,))

                coll_vars[:, 8] = np.random.uniform(e_ni_mallet[0], e_ni_mallet[1], (num_resets,))    # e_n0
                coll_vars[:, 9] = np.random.uniform(e_nr_mallet[0], e_nr_mallet[1], (num_resets,))      # e_nr  
                coll_vars[:, 10] = np.random.uniform(e_nf_mallet[0], e_nf_mallet[1], (num_resets,))
                coll_vars[:, 11] = np.random.uniform(e_ti_mallet[0], e_ti_mallet[1], (num_resets,))
                coll_vars[:, 12] = np.random.uniform(e_tr_mallet[0], e_tr_mallet[1], (num_resets,))
                coll_vars[:, 13] = np.random.uniform(e_tf_mallet[0], e_tf_mallet[1], (num_resets,))
                coll_vars[:, 14] = np.random.uniform(e_nstd_mallet[0], e_nstd_mallet[1], (num_resets,))
                coll_vars[:, 15] = np.random.uniform(e_tstd_mallet[0], e_tstd_mallet[1], (num_resets,))
...
for timestep in range(100000):
    ...
    while True:
        ...
        if next_img < next_mallet and next_img < next_action:
            ...
            if beam_interference.any():
                past_puck_data = camera_buffer.get([0])[:,:2][beam_interference] # player, x/y (2*envs,2)
                puck_pos[beam_interference] = np.where(np.random.random((int(np.sum(beam_interference)))) < percent_miss, past_puck_data, puck_pos[beam_interference])
